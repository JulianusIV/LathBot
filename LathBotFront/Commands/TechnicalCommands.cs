...
namespace LathBotFront.Commands
{
    class TechnicalCommands : BaseCommandModule
    {
        ...
        {
            var role = ctx.Guild.GetRole(714969974527229973);
            await ctx.RespondAsync(new DiscordMessageBuilder() { Content = role.Mention }.WithAllowedMention(new UserMention(228763974299156482)));
        }
        ...
        [RequireUserPermissions(DiscordPermissions.BanMembers)]
        ...
        public async Task ToggleQuestions(CommandContext ctx)
        {
            ...
            var open = perms.First(x => x.Id == 759456739669704784).Allowed.HasPermission(DiscordPermissions.SendMessages);
            foreach (var change in toChange)
                await perms.First(x => x.Id == change).UpdateAsync(open ? DiscordPermissions.None : DiscordPermissions.SendMessages,
                                                                   open ? DiscordPermissions.SendMessages : DiscordPermissions.None);
            if (open)
        ...
        private async Task FreezeChannel(DiscordChannel channel)
        {
            ...
            if (perms.Any(x => rolemeIds.Contains(x.Id)))
                await perms.First(x => x.Id == 767050052257447936).UpdateAsync(DiscordPermissions.None, DiscordPermissions.SendMessages);
            if (channel.Id == 766463841059078205) //counting
                await perms.First(x => x.Id == 767050052257447936).UpdateAsync(DiscordPermissions.None, DiscordPermissions.SendMessages);
            else
                await perms.First(x => x.Id == 767050052257447936).UpdateAsync(DiscordPermissions.AccessChannels, DiscordPermissions.SendMessages);
        ...
        private async Task UnfreezeChannel(DiscordChannel channel)
        {
            ...
            if (perms.Any(x => rolemeIds.Contains(x.Id)))
                await perms.First(x => x.Id == 767050052257447936).UpdateAsync(DiscordPermissions.SendMessages,
                    channel.Id == 718162681554534511 ? DiscordPermissions.AttachFiles | DiscordPermissions.EmbedLinks : DiscordPermissions.None); //if debate, no embeds or files
            if (channel.Id == 766463841059078205) //counting
                await perms.First(x => x.Id == 767050052257447936).UpdateAsync(DiscordPermissions.None, DiscordPermissions.EmbedLinks | DiscordPermissions.AttachFiles);
            else
                await perms.First(x => x.Id == 767050052257447936).UpdateAsync(DiscordPermissions.AccessChannels | DiscordPermissions.SendMessages, DiscordPermissions.None);
